...
hdb = dbConnect(MySQL(), dbname='hospital', host= options()$mysql$host,port= options()$mysql$port, user= options()$mysql$user, password = options()$mysql$password)

dbListTables(hdb)
#create table in R from Mysql table
myQuery <- "select * from hospital.hospital_location"
hdata <- dbGetQuery(hdb, myQuery)
str(hdata)
dbDisconnect(hdb)
